...
const ChatRoom = () => {
  ...
  const fetchUsername = async () => {
    try {
      const response = await fetch(
        `http://93.127.199.85:9090/api/ats/157industries/employeeName/${employeeId}/${userType}`
      );
  ...
  const connect = (username) => {
    let Sock = new SockJS("http://93.127.199.85:9090/ws");
  ...
  const uploadFile = () => {
    ...
    fetch("http://93.127.199.85:9090/upload", {
      method: "POST",
      body: formData,
    })
      .then((response) => {
