...
const InterviewDates = ({ toggleShowShortListedCandidateData }) => {
  ...
  const fetchAndUpdateInterviewResponse = async (
    ...
    try {
      const response = await fetch(
        `http://192.168.1.38:8891/api/ats/157industries/interview-response/${candidateId}/${employeeIdNew}/${requirementId}`
      );
  ...
  const handleInterviewResponseSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        "http://192.168.1.38:8891/api/ats/157industries/save-interview-response",
        {
          method: "POST",
          headers: {
