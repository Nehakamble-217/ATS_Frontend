...
const InterviewDates = ({ toggleShowShortListedCandidateData }) => {
  ...
  const fetchInterviewDates = async () => {
    try {
      const response = await fetch(
        `http://localhost:9090/api/ats/157industries/interview-date/${employeeIdNew}`
      );
      const data = await response.json();
  ...
  const handleDateChange = async (date) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:9090/api/ats/157industries/today-interview/${employeeIdNew}?date=${formattedDate}`
      );
      const data = await response.json();
      if (data.length === 0) {
