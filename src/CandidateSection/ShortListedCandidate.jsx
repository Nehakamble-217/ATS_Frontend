...
const ShortListedCandidates = ({ closeComponents, viewUpdatedPage}) => {
    ...
    const fetchShortListedData = async () => {  
        try {
            const response = await fetch(
                `http://192.168.1.36:8891/api/ats/157industries/shortListed-date/${newEmployeeId}`
            );
    ...
    const handleViewUpdatedPage = (candidateId) => {
        viewUpdatedPage(newEmployeeId, candidateId);
        console.log(candidateId + "-->  candidateId in shortlisted page");
        console.log(newEmployeeId + "-->  newEmployeeId in shortlisted page");
    };
                            ...
                            {shortListedData.map((item) => (
                                <tr key={item.candidateId} className="attendancerows">
                                    ...
                                    <td className="tabledata" style={{ whiteSpace: "nowrap" }} >
                                        <i onClick={() => handleViewUpdatedPage(item.candidateId)} className="fa-sharp fa-solid fa-eye"></i>
