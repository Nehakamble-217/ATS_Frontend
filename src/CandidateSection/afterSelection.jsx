...
const AfterSelection = ({
  ...
  const fetchCandidateTableData = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.1.36:9090/api/ats/157industries/fetch-after-selection?candidateId=${candidateId}&employeeId=${employeeId}&requirementId=${requirementId}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://192.168.1.36:9090/api/ats/157industries/add-after-selection",
        {
          method: "POST",
          headers: {
  ...
  const JoininghandleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://192.168.1.36:9090/api/ats/157industries/save-join-data",
        {
          method: "POST",
          body: formData,
        }
