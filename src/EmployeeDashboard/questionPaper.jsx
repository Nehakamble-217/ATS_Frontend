...
const QuestionPaper = () => {
  const [questions, setQuestions] = useState([
    { id: 1, question: 'What is the capital of France?', options: ['Paris', 'Berlin', 'London', 'Madrid'], selectedOption: '' },
    { id: 2, question: 'Who wrote "To Kill a Mockingbird"?', options: ['Harper Lee', 'Jane Austen', 'Charles Dickens', 'F. Scott Fitzgerald'], selectedOption: '' },
    { id: 3, question: 'What is the largest ocean on Earth?', options: ['Atlantic Ocean', 'Indian Ocean', 'Arctic Ocean', 'Pacific Ocean'], selectedOption: '' },
    { id: 4, question: 'Which planet is known as the Red Planet?', options: ['Mars', 'Jupiter', 'Venus', 'Mercury'], selectedOption: '' },
    { id: 5, question: 'Who painted the Mona Lisa?', options: ['Vincent van Gogh', 'Leonardo da Vinci', 'Pablo Picasso', 'Michelangelo'], selectedOption: '' },
    { id: 6, question: 'What is the currency of Japan?', options: ['Yuan', 'Dollar', 'Yen', 'Euro'], selectedOption: '' },
    { id: 7, question: 'Who discovered gravity?', options: ['Isaac Newton', 'Albert Einstein', 'Galileo Galilei', 'Stephen Hawking'], selectedOption: '' },
    { id: 8, question: 'What is the capital of Italy?', options: ['Rome', 'Madrid', 'Berlin', 'Lisbon'], selectedOption: '' },
    { id: 9, question: 'Who wrote the play "Hamlet"?', options: ['William Shakespeare', 'John Milton', 'Jane Austen', 'Charles Dickens'], selectedOption: '' },
  ...
  const handleAddQuestion = () => {
    const newQuestion = { id: questions.length + 1, question: '', options: ['', '', '', ''], selectedOption: '' };
  ...
  const generatePDF = () => {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4', hotfixes: [] });
    ...
    doc.line(centerX - textWidth / 2 - underlineMargin, underlineY, centerX + textWidth / 2 + underlineMargin, underlineY);
    ...
    doc.line(centerX - careerTextWidth / 2 - underlineMargin, careerUnderlineY, centerX + careerTextWidth / 2 + underlineMargin, careerUnderlineY);
  
    y += 10;
    doc.setFontSize(12);
    doc.text('Recruitment | Development | Training | Anticlockwise Wrist Watches', centerX, y, { align: 'center' });
    y += 10;
    doc.text('Office no 2, 2nd floor, Ace Nest Building, above cafe Milano 703, Taboot Street, Pune 411001', centerX, y, { align: 'center' });
    y += 10;
    ...
    questions.forEach((question, index) => {
      ...
      const questionFitsOnCurrentPage = currentY + spaceRequired + questionHeight <= pageHeight - 20;
      ...
      if (questionText.length > maxQuestionLength) {
        ...
        doc.text(questionText, leftColumnX, currentY, { align: 'left', maxWidth: pageWidth - 30 });
      ...
      <div className="question-paper-container">
        <div className='header-div'>
          <header className="header">
            ...
            <p>Recruitment | Development | Training | Anticlockwise Wrist Watches</p>
            <p>Office no 2, 2nd floor, Ace Nest Building, above cafe Milano 703, Taboot Street, Pune 411001</p>
          </header>
        </div>
          ...
          <div className="form-containers">
            <form className="question-paper">
              ...
              <div className="buttons">
                <button type="button" onClick={handleAddQuestion}>Add More</button>
                <button type="button" onClick={handleViewPDF}>View</button>
              </div>
            </form>
          </div>
          {showPDF && (
            <div className="pdf-preview">
              <iframe ref={iframeRef} src={pdfSrc} width="100%" height="500px" title="PDF Preview"></iframe>
              <div className="pdf-buttons">
                <button className="button" onClick={handleDownloadPDF}>Download PDF</button>
                <button className="button" onClick={handleCancelPDF}>Cancel</button>
              </div>
            </div>
          )}
          {showAlert && (
