...
const InterviewForm = ({ toggleAllInterviewResponse }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.interviewStatus === "Yes") {
        response = await axios.post('http://93.127.199.85:1414/api/interview/details', formData);
        await axios.post('http://93.127.199.85:1414/api/interview/status/yes', formData);
      } else if (formData.interviewStatus === "No") {
        response = await axios.post('http://93.127.199.85:1414/api/interview/details', formData);
        await axios.post('http://93.127.199.85:1414/api/interview/status/no', formData);
      } else if (formData.interviewStatus === "Yet to be confirmed") {
        response = await axios.post('http://93.127.199.85:1414/api/interview/details', formData);
        await axios.post('http://93.127.199.85:1414/api/interview/status/yet-to-be-confirmed', formData);
      }
  ...
  const fetchRequirementIdCandidate = async (requirementId) => {
    try {
      const response = await axios.get(
        `http://93.127.199.85:9090/api/ats/157industries/candidate-details/${requirementId}`
      );
      const { data } = response;
