...
const NotePad = () => {
  ...
  const saveMessage = async (e) => {
    ...
    try {
      let url = editMessageId ? `http://localhost:8891/api/ats/157industries/updateNoteData/${editMessageId}` : 'http://localhost:8891/api/ats/157industries/notes';
  ...
  const fetchNotePadData = async () => {
    try {
      const response = await fetch('http://localhost:8891/api/ats/157industries/notesData');
      if (!response.ok) {
  ...
  const updateMessage = async (messageId) => {
    try {
      const response = await fetch(`http://localhost:8891/api/ats/157industries/updateNoteData/${messageId}`);
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const data = await response.json();
  ...
  try {
    const response = await fetch(`http://localhost:8891/api/ats/157industries/deleteNoteData/${messageId}`, {
      method: "DELETE",
    });
    if (response.ok) {
