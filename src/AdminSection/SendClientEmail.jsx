...
const SendClientEmail = ({ clientEmailSender }) => {
  ...
  useEffect(() => {
    fetch(
      `http://192.168.1.36:9090/api/ats/157industries/calling-lineup/${employeeId}/${userType}`
    )
...
const SendEmailPopup = ({
  ...
  const handleStoreClientInformation = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.36:9090/api/ats/157industries/add-client-details",
        clientData
      );
      if (response) {
  ...
  const handleSendEmail = () => {
    ...
    axios
      .post(
        "http://192.168.1.36:9090/api/ats/157industries/send-email",
        emailData
      )
      .then((response) => {
