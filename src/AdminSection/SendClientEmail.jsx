...
const SendClientEmail = ({ clientEmailSender }) => {
  ...
  useEffect(() => {
    fetch(
      `http://192.168.1.51:9090/api/ats/157industries/calling-lineup/${employeeId}/${userType}`
    )
...
const SendEmailPopup = ({
  ...
  const handleStoreClientInformation = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.51:9090/api/ats/157industries/add-client-details",
        clientData
      );
      if (response) {
  ...
  const handleSendEmail = () => {
    ...
    axios
      .post(
        "http://192.168.1.51:9090/api/ats/157industries/send-email",
      )
      .then((response) => {
        ...
        selectedCandidate.forEach(async (can) => {
          try {
            const performanceId = await axios.get(
              `http://192.168.1.51:9090/api/ats/157industries/fetch-performance-id/${can.candidateId}`
            );
  ...
  const UpdatePerformace = async (id) => {
    try {
      ...
      const response1 = await axios.put(
        `http://192.168.1.51:9090/api/ats/157industries/update-performance/${id}`,
        additionalData
      );
