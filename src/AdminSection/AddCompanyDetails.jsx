...
const AddCompanyDetails = () => {
  ...
  const fetchPreviousCompanyDetailsId = async () => {
    const response = await axios.get(
      "http://192.168.1.51:9090/api/ats/157industries/fetch-details-ids"
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.51:9090/api/ats/157industries/save-our-company",
        initialFormData,
        {
          headers: {
...
const SendEmailPopup = ({
    ...
    const fetchCompanyDetailsById = async () => {
      ...
      if (onOptionChange != null) {
        response = await axios.get(
          `http://192.168.1.51:9090/api/ats/157industries/details-by-Id/${onOptionChange}`
        );
      } else {
        response = await axios.get(
          `http://192.168.1.51:9090/api/ats/157industries/details-by-Id/${latestAddedData}`
        );
      }
  ...
  const handleStoreClientInformation = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.51:9090/api/ats/157industries/save-send-details",
        clientData
      );
      if (response) {
  ...
  const handleSendEmail = () => {
    ...
    axios
      .post(
        "http://192.168.1.51:9090/api/ats/157industries/company-detail-email",
        emailData
      )
      .then((response) => {
